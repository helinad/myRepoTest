...
public class PopoverView extends RelativeLayout implements OnTouchListener{
	 ...
	 */
	public static int defaultPopoverBackgroundDrawable = R.drawable.large_popover_black_bcg;
	/**
	 * The default popover arrow up drawable for all the popovers
	 */
	public static int defaultPopoverArrowUpDrawable = R.drawable.popover_black_toup_arrow;
	/**
	 * The default popover arrow down drawable for all the popovers
	 */
	public static int defaultPopoverArrowDownDrawable = R.drawable.popover_black_bottom_arrow;
	/**
	 * The default popover arrow left drawable for all the popovers
	 */
	public static int defaultPopoverArrowLeftDrawable = R.drawable.popover_black_left_arrow;
	/**
	 * The default popover arrow down drawable for all the popovers
	 */
	public static int defaultPopoverArrowRightDrawable = R.drawable.popover_black_right_arrow;
	
	public static int whitedownarrow = R.drawable.glass_down_arrow;
	...
	public static Rect getFrame(View v){
		viewWidth = v.getWidth();
		int location [] = new int [2];
		v.getLocationOnScreen(location);
		Rect viewRect = new Rect(location[0], location[1], (int) (location[0]+(v.getWidth()/1.5)), (int) (location[1]+(v.getHeight()/1.5)));
		return viewRect;
	}
	 ...
	 */
	public PopoverView(Context context, int layoutId, boolean isBg) {
		super(context);
		initPopoverView(inflate(context, layoutId, null), isBg);
	}
	 ...
	 * @param layoutId The ID of the layout we want to put inside the popover
	 */
	public PopoverView(Context context, AttributeSet attrs, int layoutId, boolean isBg) {
		super(context, attrs);
		initPopoverView(inflate(context, layoutId, null), isBg);
	}
	 ...
	 * @param layoutId The ID of the layout we want to put inside the popover
	 */
	public PopoverView(Context context, AttributeSet attrs, int defStyle, int layoutId, boolean isBg) {
	 ...
	 */
	public PopoverView(Context context, View popoverView, boolean isBg) {
		super(context);
		initPopoverView(popoverView, isBg);
	}
	 ...
	 * @param popoverView The inner view we want to show in a popover
	 */
	public PopoverView(Context context, AttributeSet attrs, View popoverView, boolean isBg) {
		super(context, attrs);
		initPopoverView(popoverView, isBg);
	}
	 ...
	 * @param popoverView The inner view we want to show in a popover
	 */
	public PopoverView(Context context, AttributeSet attrs, int defStyle, View popoverView, boolean isBg) {
		super(context, attrs, defStyle);
		initPopoverView(popoverView, isBg);
	}
	 ...
	 */
	private void initPopoverView(View viewToEnclose, boolean isBg){
		...
		isCalendar = isBg;
		if(isBg){
			popoverView.setBackgroundDrawable(getResources().getDrawable(R.drawable.pop_up));
		}else{
	...
	private void addArrow(Rect originRect, Integer arrowDirection, boolean calendar){
		//Add arrow drawable
		...
		if (arrowDirection == PopoverView.PopoverArrowDirectionUp){
			...
			if(isCalendar){
				arrowDrawable = getResources().getDrawable(R.drawable.glass_down_arrow);
			}else{
		...
		if(isCalendar){
			...
			arrowParams.leftMargin = xPos-12;
			arrowParams.topMargin = yPos-12;
		}
		//add view :)
	...
	public void setContentSizeForViewInPopover(WindowManager wm, float widthFactor, float heightFactor) {
		...
		float width = popoverView.getWidth() + size.x;
		float height = popoverView.getHeight() + size.y;

		setContentSizeForViewInPopover(new Point((int)(width / widthFactor),
	...
	public void showPopoverFromRectInViewGroup(ViewGroup group, Rect originRect, int arrowDirections, boolean animated){
		...
		//Add arrow image
		addArrow(originRect, best, isCalendar);
	...
	public void update(View view){
		popoverView.updateViewLayout(view, getLayoutParams());
	}
