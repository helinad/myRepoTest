...
import com.constant_therapy.constantTherapy.ClinicianActivity;
...
public class MultiChartViewControl extends View implements ChartChangeListener,
		ChartProgressListener {

	/** The user interface thread handler. */
	private Handler mHandler;

	public MultiChartViewControl(Context context) {
		super(context);
		mHandler = new Handler();
		this.initialize();
	}

	public MultiChartViewControl(Context context, AttributeSet attrs) {
		super(context, attrs);
		mHandler = new Handler();
		this.initialize();
	}
	...
	protected static Date minimumDate;

	protected static Date maximumDate;

	private boolean isReplicate = false;

	private boolean isFirstTouch = true;

	protected static int gapValue;
	...
	private void move(PointF source, double moveBoundX, double moveBoundY,
		...
		if (p instanceof Movable) {
			...
			// p.setNotify(false);
			Date minDate = ((DateAxis) this.chart.getXYPlot().getDomainAxis())
					.getMinimumDate();
			Date maxDate = ((DateAxis) this.chart.getXYPlot().getDomainAxis())
					.getMaximumDate();

			if ((maxDate.before(maximumDate) && hMovePercent > 0)
					|| (minDate.after(minimumDate) && hMovePercent < 0)) {
		...
		if (plot instanceof Zoomable) {
			...
			((DateAxis) this.getChart().getXYPlot().getDomainAxis())
					.setTickUnit(new DateTickUnit(DateTickUnitType.DAY,
							gapValue, new SimpleDateFormat("MM/dd")));
	 ...
	 * 
	 * // first determine the size of the chart rendering area... Dimension size
	 * = getSize(); RectangleInsets insets = getInsets(); RectShape available =
	 * new RectShape(insets.getLeft(), insets.getTop(), size.getWidth() -
	 * insets.getLeft() - insets.getRight(), size.getHeight() - insets.getTop()
	 * - insets.getBottom());
	 * 
	 * double drawWidth = available.getWidth(); double drawHeight =
	 * available.getHeight(); this.scaleX = 1.0; this.scaleY = 1.0;
	 * 
	 * if (drawWidth < this.minimumDrawWidth) { this.scaleX = drawWidth /
	 * this.minimumDrawWidth; drawWidth = this.minimumDrawWidth; } else if
	 * (drawWidth > this.maximumDrawWidth) { this.scaleX = drawWidth /
	 * this.maximumDrawWidth; drawWidth = this.maximumDrawWidth; }
	 * 
	 * if (drawHeight < this.minimumDrawHeight) { this.scaleY = drawHeight /
	 * this.minimumDrawHeight; drawHeight = this.minimumDrawHeight; } else if
	 * (drawHeight > this.maximumDrawHeight) { this.scaleY = drawHeight /
	 * this.maximumDrawHeight; drawHeight = this.maximumDrawHeight; }
	 * 
	 * RectShape chartArea = new RectShape(0.0, 0.0, drawWidth, drawHeight);
	 * 
	 * // are we using the chart buffer? // if (this.useBuffer) { // // // do we
	 * need to resize the buffer? // if ((this.chartBuffer == null) // ||
	 ...
	 * this.refreshBuffer = true; // } // // // do we need to redraw the buffer?
	 * // if (this.refreshBuffer) { // // this.refreshBuffer = false; // clear
	 * the flag // // RectShape bufferArea = new RectShape( // 0, 0,
	 * this.chartBufferWidth, this.chartBufferHeight); // // Graphics2D bufferG2
	 * = (Graphics2D) // this.chartBuffer.getGraphics(); // RectShape r = new
	 * RectShape(0, 0, this.chartBufferWidth, // this.chartBufferHeight); //
	 * bufferG2.setPaint(getBackground()); // bufferG2.fill(r); // if (scale) {
	 * // AffineTransform saved = bufferG2.getTransform(); // AffineTransform st
	 * = AffineTransform.getScaleInstance( // this.scaleX, this.scaleY); //
	 * bufferG2.transform(st); // this.chart.draw(bufferG2, chartArea,
	 * this.anchor, // this.info); // bufferG2.setTransform(saved); // } // else
	 ...
	 * }
	 * 
	 * // TODO:AffineTransform // or redrawing the chart every time... // else {
	 * 
	 * // AffineTransform saved = canvas.getTransform(); //
	 * canvas.translate(insets.left, insets.top); // if (scale) { //
	 * AffineTransform st = AffineTransform.getScaleInstance( // this.scaleX,
	 * this.scaleY); // canvas.transform(st); // } // this.chart.draw(canvas,
	 * chartArea, this.anchor, this.info); // canvas.setTransform(saved);
	 * 
	 * // } this.chart.draw(canvas, chartArea, this.anchor, this.info);
	 * 
	 * 
	 * // Iterator iterator = this.overlays.iterator(); // while
	 * (iterator.hasNext()) { // Overlay overlay = (Overlay) iterator.next(); //
	 * overlay.paintOverlay(canvas, this); // }
	 * 
	 * // redraw the zoom RectShape (if present) - if useBuffer is false, // we
	 * use XOR so we can XOR the RectShape away again without redrawing // the
	 * chart //drawZoomRectShape(canvas, !this.useBuffer);
	 * 
	 * //canvas.dispose();
	 * 
	 * this.anchor = null; // this.verticalTraceLine = null; //
	...
	private void zoom(PointF source, double startDistance, double endDistance) {
		...
		PlotRenderingInfo info = this.info.getPlotInfo();
		/*
		 * int dataSetCount = (this.getChart().getXYPlot().getDataset(0)
		 * .getItemCount(0) / 7) + 10;
		 */

		float width = info.getDataArea().getWidth();
	...
	public void touched(MotionEvent event) {
		...
		// String label = this.chart.getXYPlot().getRangeAxis().getLabel();
