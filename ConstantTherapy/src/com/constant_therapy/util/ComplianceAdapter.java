...
public class ComplianceAdapter  extends BaseAdapter {
	...
	private List<Compliance> list = new ArrayList<Compliance>();
	...
	public ComplianceAdapter(Context c, int month, int year, DisplayMetrics metrics) {
		mContext = c;
		mMonth = month;
		mYear = year;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			cell.tv_temp = (CTTextView) convertView
					.findViewById(R.id.calendar_cell_view);
			cell.tv.setText(mItems.get(position));
			cell.tv_temp.setText(mItems.get(position));
		} else {
		...
		if (position <= 6) {
			...
			cell.tv_temp.setHeight(mTitleHeight);
			...
			cell.tv_temp.setHeight((int) (mDayHeight / 1.5));
			cell.tv.setTextSize(12);
			cell.tv.setBackground(null);
		} else if (position <= mDaysShown - mDaysNextMonth) {
			// current month
			cell.tv.setHeight((int) (mDayHeight / 1.5));
			cell.tv_temp.setHeight((int) (mDayHeight / 1.5));
			day = position - (mDaysLastMonth + 6);
			cell.tv.setTextSize(12);
			
			if(mMonth == current.get(Calendar.MONTH) && day > current.get(Calendar.DAY_OF_MONTH) ){
		...
		GridView lvDocument = (GridView) parent;
		
		if (lvDocument.isItemChecked(position) && position > 6) {
			
			((GridView) parent).setItemChecked(position, true);
			cell.tv.setBackground(mContext.getResources().getDrawable(R.drawable.cell_gradient));
			list.add(getCompilance(Integer.parseInt(mItems.get(position)), position));
		} else if(position <= 6){
			cell.tv.setBackgroundColor(mContext.getResources().getColor(
					R.color.bg_light_gray));
		} else{
			...
			list.remove(getCompilance(Integer.parseInt(mItems.get(position)), position));
		}
	...
	public List<Compliance> getSelectedItem() {
